...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/home/shenhm/documents/watermark-stealing/out_llama/minhash_prf4-4/9/ours/c4-kgw-ff-anchored_minhash_prf4-4-True-15485863/0.jsonl",
        help="Path to the data file containing the z-scores"
    )
    parser.add_argument(
        "--config_path",
        type=str,
        default="/home/shenhm/documents/lm-watermarking/watermark_reliability_release/output/c4/KWG_TEST/len_250/llama_7B_N500_T200_no_filter_batch_1_delta_4_gamma_0.25_KWG_ff-anchored_minhash_prf4-4-True-15485863/gen_table_meta.json",
    )
    parser.add_argument(
        "--seeding_scheme",
        type=str,
...
def main():
    ...
    for item in tqdm(data):
        for key in ["textwm"]:
            input_text = item.get(key, "").strip()  # 获取并去除首尾空白字符
    ...
    pdb.set_trace()
    print(np.nanmean(np.array([item["textwm_z_score"] for item in data if "textwm_z_score" in item])))
