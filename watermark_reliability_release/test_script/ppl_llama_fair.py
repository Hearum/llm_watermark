...
def extract_ppl_dict(data, key='w_wm_output_ppl'):
    return {item['idx']: item.get(key, float('nan')) for item in data if 'idx' in item}

def compare_two_files(path1, path2, key='w_wm_output_ppl'):
    ...
    print(f"{path1} 平均 PPL: {np.mean(ppl_values1):.4f}")
    print(f"{path2} 平均 PPL: {np.mean(ppl_values2):.4f}")
    print(f"PPL 平均差 (方案1 - 方案2): {np.mean(diffs):.4f}")
    print(f"PPL 差值标准差: {np.std(diffs):.4f}")

if __name__ == '__main__':

    path1 = "/home/shenhm/documents/temp/c4/PPL_KWG_TEST/len_150/llama_7B_N500_T200_no_filter_batch_1_delta_5_gamma_0.25_KWG_ff-anchored_minhash_prf-4-True-15485863/gen_table.jsonl_gpt2"
    path2 = "/home/shenhm/documents/temp/c4/PPL_KWG_TEST/len_150/llama_7B_N500_T200_no_filter_batch_1_delta_5_gamma_0.25_KWG_ff-anchored_minhash_prf1-4-True-15485863/gen_table.jsonl_gpt2"
